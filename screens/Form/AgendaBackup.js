...
export default class Agenda extends Component {
	constructor(props){
		...
		this.state={
			showDate:false,
			showTime:false,
			id_agenda:typeof(data) == 'undefined' ? '' : data.id,
			judul_agenda: typeof(data) == 'undefined' ? '' : data.judul_agenda,
			kode_agenda:typeof(data) == 'undefined' ? '' : data.kode_agenda,
			lokasi:typeof(data) == 'undefined' ? '' : data.lokasi,
			tanggal_agenda:typeof(data) == 'undefined' ? '' : data.tanggal_agenda,
			waktu_agenda:typeof(data) == 'undefined' ? '' : data.waktu_agenda,
			action : '',
			error: '',
		}
		...
		<View style={container}>
		  <Container >
		    <Content >
			  ...
			  :
			  <View style={buttonRow}>
			    <View style={buttonRow1}>
				  <Button iconLefteft primary transparent bordered style={buttonStly} onPress={()=>this.formTrigger('Direksi')} >
					<Icon name='person-add-outline' style={{fontSize:40, color:'#0491ac'}}/>
					<Text style={{fontSize:15, color:'#0491ac'}}>Direksi</Text>
				  </Button>
				   <Button iconLeft primary transparent bordered style={buttonStly} onPress={()=>this.formTrigger('Protokol')} >
					<Icon name='person-add-outline' style={{fontSize:40, color:'#0491ac'}}/>
					<Text style={{fontSize:15, color:'#0491ac'}}>Protokol</Text>
				  </Button>
				</View>
